...
#dataDfList.append(pd.read_excel(pd.ExcelFile("/home/samaneh/AHRD/outputs/xlsx/test_evaluator_output_nrSprotExpGOAv2_against_sprot.xlsx")))
dataDfList.append(pd.read_excel(pd.ExcelFile("/home/samaneh/eggNOG/AHRD/output/evaluator_output_sprot_clstrd_with_competitors.xlsx")))


colnames = pd.Series(["AHRD Score","tair","sprot","eggNOG","myAlgo"])
...
for i in range(0,1):
	...
	eggnogSeries = pd.Series(dataDfList[i]["eggNOGmapper-Evaluation-Score"], index = rownames)
	myAlgoSeries = pd.Series(dataDfList[i]["myAlgo-Evaluation-Score"], index = rownames)
	dataDfList[i] = dataDfList[i]["Evaluation-Score"] #filter columns except evaluation scores	
	dataDfList[i]["eggNOG-mapper"] = eggnogSeries
	dataDfList[i]["myAlgorithm"] = myAlgoSeries
